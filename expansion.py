...
def Minjoin(S, K, T, i, j, len_index):
    O = set()
    C = []
    A = []
    ...
    for (i, j) in C:
        if count[(i, j)] >= int(threshold):
            num_segments = count[(i, j)]
            O.add((i, j))

    return (O, A, num_segments, len_index)
...
def compare_windows(windows1, windows2, K, T, len_index):
    results = []
    for i, win1 in enumerate(windows1):
        for j, win2 in enumerate(windows2):
            (O, A, num_segments, len_index) = Minjoin([win1, win2], K, T, i, j, len_index)
            if O:
                results.append(O)
    return results, len_index

def process_pair(pair, K, T, len_index):
        ...
        if chrom1 == chrom2:
            ...
            comparisons, lens = compare_windows(windows1, windows2, K, T, len_index)
            return comparisons, lens
...
if __name__ == "__main__":
    ...
    pairs_extended = [(sequences_extended[i], sequences_extended[i + 1]) for i in range(0, len(sequences_extended) - 1, 2)]
    ...
    len_index = [0 for i in range(10000)]
    for i, pair in enumerate(pairs):
        result, len_index = process_pair(pair, K, T, len_index)
    ...
    with open('lengths.txt', 'w') as file:
        file.write(", ".join(map(str, len_index)) + "\n")
